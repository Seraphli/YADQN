...
class MLPDQN(object):
        ...
        with tf.variable_scope('layer1'):
            for _ in range(shape[1]):
                with tf.variable_scope('part%d' % _):
                    ...
                    y += [_y]
                    weights += _w
            layer1 = tf.reshape(tf.stack(y, axis=1), [-1, shape[1] * 4])
        y = []
        with tf.variable_scope('layer2'):
            for _ in range(shape[2]):
                with tf.variable_scope('part%d' % _):
                    _y, _w = AND(layer1, [shape[1] * 4])
                    y += [_y]
                    weights += _w
                    _y, _w = OR(layer1, [shape[1] * 4])
                    y += [_y]
                    weights += _w
                    _y, _w = NOT(layer1, [shape[1] * 4])
                    y += [_y]
                    weights += _w
                    _y, _w = XOR(layer1, [shape[1] * 4])
                    y += [_y]
                    weights += _w
            y = tf.reshape(tf.stack(y, axis=1), [-1, shape[2] * 4])
    ...
    def build_graph(self):
        ...
        network_shape = [list(self._env.observation_space.shape)[0], 32, 32, self._env.action_space.n]
        ...
        with tf.variable_scope('q_target_net', reuse=False):
            q_target, q_target_w = self._def_layers(s_, network_shape)

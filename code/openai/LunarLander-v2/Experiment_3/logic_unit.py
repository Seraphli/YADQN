...
def AND(x, shape):
    with tf.variable_scope('AND'):
        w = tf.get_variable('w', shape, tf.float32, initializer=w_init)
        b = tf.get_variable('b', shape, tf.float32, initializer=b_init)
...
def NOT(x, shape):
    with tf.variable_scope('NOT'):
        w = tf.get_variable('w', shape + [1], tf.float32, initializer=w_init)
        b = tf.get_variable('b', [1], tf.float32, initializer=b_init)
